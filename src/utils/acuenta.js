...
async function getDataFromAcuenta() {
  ...
  try {
    await page.goto('https://www.acuenta.cl/store/580__843/xlarge', {
    ...
    const products = await page.evaluate(() => {
      const items = Array.from(
        document.querySelectorAll('.card-product-vertical'),
      )
      return items
        .map((item) => {
          try {
            const cartQuantifier = item.querySelector('[data-dtname]')
            const id = cartQuantifier
              ? cartQuantifier.getAttribute('data-dtname')
              : ''
            ...
            const promotion = promoElement
              ? promoElement.textContent.trim()
              : ''
